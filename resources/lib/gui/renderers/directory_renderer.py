...
class DirectoryRenderer(Renderer):
    ...
    def a_to_z_menu(self, collection):
        ...
        with self.start_directory(self.handle):
            DirectoryItem(title=self._a_to_z_title(get_string(30251), letter_counts.get(zero_nine_regex)),
    ...
    def a_to_z_submenu(self, collection, previous_letter):
        ...
        with self.start_directory(self.handle):
            MainMenuFolderItem(url=router_url_from_string(ROUTE.MAIN_MENU))(self.handle)
            SearchItem(title=get_string(30252).format(letters=previous_letter),
    ...
    def _a_to_z_submenu_items(self, collection, filter_type, previous_letter, letter_counts):
        ...
        for c in string.ascii_uppercase:
            ...
            if count > 0:
                url = router_url_from_string(ROUTE.FILTER, collection, filter_type,
                                             letters) if count == 1 else self._url_for(self.a_to_z_submenu,
                                                                                       collection, letters)
